 ...
 * 96px desktop top bar; values match the Figma frame. On mobile the map runs edge to edge
 ...
 */
export const FRAME_INSETS = `fixed inset-x-0 ${BELOW_MOBILE_HEADER} bottom-0 md:inset-x-6 md:top-[96px] md:bottom-8`;

/** Landing globe: smaller page margin so the sphere reads larger on desktop. */
export const GLOBE_BACKDROP_INSETS = `fixed inset-x-0 ${BELOW_MOBILE_HEADER} bottom-0 md:inset-x-2 md:top-[96px] md:bottom-3`;
