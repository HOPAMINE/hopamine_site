...
const FADE_SPAN = 1;
const fadeIn = (full: number): ExpressionSpecification => [
  "interpolate",
  ["linear"],
  ["zoom"],
  MINIMAL_BELOW_ZOOM,
  0,
  MINIMAL_BELOW_ZOOM + FADE_SPAN,
  full,
];
 ...
 */
export function buildGlobeStyle(p: GlobePalette): StyleSpecification {
  return {
    version: 8,
    ...
    layers: [
      ...
      },
      {
        ...
        filter: [
          "match",
          ["get", "class"],
          ["primary", "secondary", "tertiary", "trunk", "minor"],
          true,
          false,
        ],
        layout: {
